...
const App = () => {
  ...
  const acceptStartup = (startupId) => {
    ...
    if (selectedStartup) {
      ...
      setAcceptedStartups((prevAcceptedStartups) => [
        ...prevAcceptedStartups,
        selectedStartup,
      ]);
    } else {
      const selectedStartup = discardedStartups.find(
        (startup) => startup.id === startupId
      );
      if (selectedStartup) {
        setDiscardedStartups((prevDiscardedStartups) =>
          prevDiscardedStartups.filter((startup) => startup.id !== startupId)
        );
        setAcceptedStartups((prevAcceptedStartups) => [
          ...prevAcceptedStartups,
          selectedStartup,
  ...
  const discardStartup = (startupId) => {
    const selectedStartup = acceptedStartups.find(
      (startup) => startup.id === startupId
    );
    if (selectedStartup) {
      setAcceptedStartups((prevAcceptedStartups) =>
    ...
    <div className="flex flex-col items-center justify-center bg-gray-100">
      ...
      >
        {showDiscardedStartups
          ? "Hide Discarded Startups"
          : "Show Discarded Startups"}
      </button>
      ...
      >
        <Sheet variant="outlined" className="rounded-md p-6 shadow-lg">
          ...
          <div className="mt-4">
            ...
            {acceptedStartups.some(
              (startup) => startup.id === selectedStartup?.id
            ) && (
              <button
                onClick={() => discardStartup(selectedStartup?.id)}
                className="bg-red-500 hover:bg-red-600 hover:border-red-700 text-white"
